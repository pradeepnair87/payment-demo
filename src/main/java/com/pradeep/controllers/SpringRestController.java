...
public class SpringRestController {
	...
	@Autowired
	public SpringRestController(BankService bankService, TransactionHistoryService transactionHistoryService,
			TransferServiceHandler transferServiceHandler) {
	...
	@ResponseBody
	public ResponseDetails createAccount(
			@RequestParam(required = true) @Size(min = 6, max = 20, message = "guest name length must be between 8 and 20") String guestName,
			@RequestParam(required = true) Long intialDeposit) {
		ThreadContext.clearMap();
		ThreadContext.put("txnId", UUID.randomUUID().toString());
		log.debug("Received Create Request for user {} and deposit {}",
				guestName.substring(guestName.length() - 4, guestName.length()), intialDeposit);
	...
	@ResponseBody
	public UserResponseDetails viewAccount(
			@PathVariable(value = "name") @Size(min = 6, max = 20, message = "guest name length must be between 8 and 20") String guestName) {
		ThreadContext.clearMap();
		ThreadContext.put("txnId", UUID.randomUUID().toString());
		log.debug("Received View Request for user {} ",
				guestName.substring(guestName.length() - 4, guestName.length()));
		UserAccount account = bankService.viewAccountDetials(guestName);
		log.info("Finished Viewing user");
		UserResponseDetails response = UserResponseDetails.builder().balance(account.getBalance())
				.name(account.getName()).id(account.getId()).build();
		return response;
	}
	...
	public ResponseDetails transferFund(@RequestBody TransferRequest transferRequest) {
		...
		return transferServiceHandler.handleTransfer(transferRequest);
