...
public TransactionResponse transferFund(UserAccount fromAccount, UserAccount toAccount, Long  amount){
    ...
    if (fromAccount.getId() < toAccount.getId()) {
        ...
        //Will never happen as Account Validation will ensure fromAccount and toAccount are different
...
private TransactionResponse doTransferInternal(UserAccount fromAccount, UserAccount toAccount, Long  amount){
    ...
    try{
        ...
        return TransactionResponse.builder().Amount(amount).fromAcount(fromAccount).toAccount(toAccount).message(ExceptionCodes.INSUFFICIENT_FUND_RESP_MSG).status(Status.FAILED).code(ExceptionCodes.InsufficientFundExceptionCode).build();
    }
    ...
    return TransactionResponse.builder().Amount(amount).fromAcount(fromAccount).toAccount(toAccount).message(ExceptionCodes.TRANSFER_SUCCESS_RESP_MSG).status(Status.SUCCESS).code(ExceptionCodes.Success).build();
}
